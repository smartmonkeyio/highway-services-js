...
export const ServiceSchema = Joi.object().keys({
  ...
  timewindows: Joi.array()
    .items(validators.timewindow())
    .single(),
  reward: Joi.number()
    .min(1)
    .description(
      `Reward obtained when performing de task, below 1 won't be assigned`,
    ),
...
export const VehicleListSchema = Joi.array()
  .items(VehicleSchema)
  .min(1);
export const ServiceListSchema = Joi.array()
  .items(ServiceSchema)
  .min(1);
...
export function validateRewardRegions(rewardRegions: any) {
  ...
  rewardRegions.forEach((value: object, idx: number) => {
    ...
    if (result.error) {
      throw new Error(
        `Error in rewardRegion [${idx}]: ${result.error.message}`,
      );
    }
  });
}
